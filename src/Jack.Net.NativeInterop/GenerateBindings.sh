...
PACKAGE_ARCH=$(dotnet msbuild "${SCRIPT_DIR}/Jack.Net.NativeInterop.csproj" -getProperty:NETCoreSdkRuntimeIdentifier)
...
#  echo ">" ln -s "${LIBCLANG_LOCATION}" "${LIBCLANG_LINK_TARGET}"
#  ln -s "${LIBCLANG_LOCATION}" "${LIBCLANG_LINK_TARGET}"
fi
...
#  echo ">" ln -s "${LIBCLANGSHARP_LOCATION}" "${LIBCLANGSHARP_LINK_TARGET}"
